...
void main() {
  ...
  test(
    'fetchConversations calls remote data source and returns conversations',
    () async {
      final conversations = [
      ...
      when(
        mockRemoteDataSource.loadConversations(),
  ...
  test(
    'getConversationById calls remote data source and returns conversation',
    () {
      ...
      when(
        mockRemoteDataSource.getConversationById('c1'),
  ...
  test(
    'saveConversations calls remote data source with correct parameter',
    () async {
      final conversations = [
        ConversationModel(conversationId: 'c1', ownerId: 'o1', messages: []),
      ];
      when(
        mockRemoteDataSource.saveConversations(conversations),
      ).thenAnswer((_) async {});
  ...
  test(
    'getConversationByOwnerId calls remote data source and returns conversation',
    () {
      final conversation = ConversationModel(
        conversationId: 'c2',
        ownerId: 'o2',
      ...
      when(
        mockRemoteDataSource.getConversationByOwnerId('o2'),
      ).thenReturn(conversation);
