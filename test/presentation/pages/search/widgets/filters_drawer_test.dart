...
void main() {
  ...
  Widget buildTestWidget() {
    return MaterialApp(
      home: BlocProvider<SearchPageCubit>.value(value: mockCubit, child: const FiltersDrawer()),
    );
  }
  ...
  testWidgets('renders FiltersDrawer and displays initial values', (tester) async {
    ...
    expect(find.text(AppLocalisations.searchFilterParametersTitle), findsOneWidget);
    ...
    expect(find.text(AppLocalisations.bodyTypeSedan), findsOneWidget);
    final sedanCheckbox = find.byWidgetPredicate(
      (widget) =>
          widget is CheckboxListTile &&
          widget.title is Text &&
          (widget.title as Text).data == AppLocalisations.bodyTypeSedan,
    );
  ...
  testWidgets('calls cubit methods when body type checkbox is toggled', (tester) async {
    AppLocalisations.localisations = {'filters.parameters.bodyTypes.sedan': 'Sedan'};
  ...
  testWidgets('calls cubit methods when fuel type checkbox is toggled', (tester) async {
    AppLocalisations.localisations = {'filters.parameters.fuelTypes.diesel': 'Diesel'};
    ...
    final dieselCheckbox = find.byWidgetPredicate(
      (widget) =>
          widget is CheckboxListTile &&
          widget.title is Text &&
          (widget.title as Text).data == AppLocalisations.fuelTypeDiesel,
    );
